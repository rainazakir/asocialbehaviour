...
#         negative[i+1] = -1
    ...
    spd = [ [0]*(sum(state[0:3])+1) for _ in range(sum(state[0:3])+1) ]
    ...
    for a in range( sum(state[0:3])+1 ):
        for b in range( sum(state[0:3])+1 ):
            spd[ a ][ b ] /= T
    ...
    if (finalStateFile != "none"):
        ...
        with open(finalStateFile, "a") as f:
            ...
            out += str(t) + "\t" + '\t'.join(str(x) for x in state) + "\n"
            f.write(out)
    out = str(valuesA[0])+ "\t" + "1"+ "\t" + str(t) + "\t" + '\t'.join(str(x) for x in state)
    print(out)
    # if DEBUG:
    #     out = '\t'.join(str(x) for x in extraLog)
    #     if (len(out)>0): out += '\t'
    ...
    if (plot_evo):
        if (temporalEvolution == "none"):
            ...
            plotit(temporalEvolution,T,state,N,evoStream,Z,SA,SB,theAavg,theBAvg,valuesA[0])

##################################################

## TODO: Specify which are the expected input arguments 

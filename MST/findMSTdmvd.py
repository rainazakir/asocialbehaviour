...
#    print("VoC:", vectorsOfChange)
    ...
    if (plot_evo):
       if (temporalEvolution == "none"):
           ...
           if whichMST == "AtoB":
               plotit(temporalEvolution,T,state,N,evoStream,Z,SA,SB,0,Avg,rate)
           else:
               plotit(temporalEvolution,T,state,N,evoStream,Z,SA,SB,Avg,0,rate)
...
#     valuesB = np.arange(6.5, 15.01, 0.1)
    ...
    S = N-Z
    print(N,Z,"Find DMVD: ", whichMST, "Average: ", Avg, "QRatio: ", rate, "Repetition: ", repetitions)
    print(r)
    state = [SA] + [SB] + [Z/2] + [Z/2]
...
#         valuesB = np.arange(valueA-3, valueA+3.01, 0.1)
        ...
        for valueB in valuesB:
            ...
            for i in range(0,repetitions):
                #temporalEvolution = 'none'
                temporalEvolution = 'popevocdci/evo-NNN' + str(N) + '-v' + str(valueA) + "-" + str(valueB) + "-" + str(i) +  '.txt'
